                ...
                <div class="tab-content py-3">
                    <div class="tab-pane fade show active" id="successhome" role="tabpanel">
                            ...
                            <ul>
                                @php
                                    $employeeManagementRoutes = [
                                        'user_access' => 'admin.users.index',
                                        'target_access' => 'admin.targets.index',
                                        'shift_access' => 'admin.shifts.index',
                                        'attendance_access' => 'admin.attendances.index',
                                        'attendance_schedule_access' => 'admin.attendance.schedule.index',
                                        'Request_Type_access' => 'admin.request-types.index',
                                        'Requests_access' => 'admin.requests.create',
                                    ];
                                @endphp
                                <x-menu.single-link
                                    :label="trans('cruds.employeeManagement.title')"
                                    icon="manage_accounts"
                                    :route="getFirstPermittedRoute($employeeManagementRoutes)"
                                    :active="request()->routeIs(['admin.users.*','admin.targets.*','admin.shifts.*','admin.attendances.*','admin.attendance.schedule.*','admin.request-types.*','admin.requests.*'])" />


                                @canany(['task_management_access', 'task_status_access', 'task_tag_access', 'task_access', 'tasks_calendar_access'])
                                    @php
                                        $taskManagementRoutes = [
                                            'task_access' => 'admin.tasks.index',
                                            'task_status_access' => 'admin.task-statuses.index',
                                            'task_tag_access' => 'admin.task-tags.index',
                                            'tasks_calendar_access' => 'admin.tasks-calendars.index',
                                        ];
                                    @endphp
                                    <x-menu.single-link
                                        :label="trans('cruds.taskManagement.title')"
                                        icon="task_alt"
                                        :route="getFirstPermittedRoute($taskManagementRoutes)"
                        ...
                        @canany(['driver_access', 'order_access'])
                            @php
                                $driverRoutes = [
                                    'driver_access' => 'admin.drivers.index',
                                    'order_access' => 'admin.orders.index',
                                ];
                            @endphp
                            <x-menu.single-link
                                :label="trans('cruds.driver.title_singular')"
                                icon="directions_bike"
                                :route="getFirstPermittedRoute($driverRoutes)"
                        ...
                        <li>
                            ...
                            <ul>
                                ...
                                @php
                                    $expenseUrl = '#';
                                    if (auth()->user()->can('app_expense_access')) {
                                        $expenseUrl = route('admin.app-expenses.index', ['type' => 'operational']);
                                    } elseif (auth()->user()->can('app_expense_source_access') && Route::has('admin.app-expense-sources.index')) {
                                        $expenseUrl = route('admin.app-expense-sources.index');
                                    }
                                @endphp
                                <x-menu.single-link
                            ...
                            @endcanany
                            @canany(['app_revenue_access', 'revenue_source_access'])
                            @php
                                $revenueUrl = '#';
                                if (auth()->user()->can('app_revenue_access')) {
                                    $revenueUrl = route('admin.app-revenues.index', ['type' => 'contract']);
                                } elseif (auth()->user()->can('revenue_source_access') && Route::has('admin.revenue-sources.index')) {
                                    $revenueUrl = route('admin.revenue-sources.index');
                                }
                            @endphp
                            <x-menu.single-link
                        ...
                        @canany(['client_access', 'contact_company_access', 'restaurant_access'])
                            @php
                                $subscriptionRoutes = [
                                    'client_access' => 'admin.clients.index',
                                    'contact_company_access' => 'admin.contact-companies.index',
                                    'restaurant_access' => 'admin.restaurants.index',
                                ];
                            @endphp
                            <x-menu.single-link
                                :label="trans('cruds.subscriptions.title_singular')"
                                :route="getFirstPermittedRoute($subscriptionRoutes)"
                                icon="person"
                        ...
                        $securityControlRoutes = [
                            'security_and_control_user_edit' => 'admin.user-setup.setup',
                            'security_and_control_role_create' => 'admin.role-setup.create',

                        ];
                    @endphp
                        <x-menu.single-link
                            :label="trans('cruds.security_and_control.title_singular')"
                             :route="getFirstPermittedRoute($securityControlRoutes)"
                    ...
                    <div class="tab-pane fade" id="successcontact" role="tabpanel">
                        @canany(['audit_log_access'])
                            @php
                                $userManagementRoutes = [
                                    'audit_log_access' => 'admin.audit-logs.index',
                                ];
                            @endphp
                            <x-menu.single-link
                                :label="trans('cruds.userManagement.title')"
                                icon="group"
                                :route="getFirstPermittedRoute($userManagementRoutes)"
